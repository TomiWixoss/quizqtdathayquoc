...
// AUTO-GENERATED từ public/AI/*.js bởi scripts/sync-inline-workers.cjs
// KHÔNG CHỈNH SỬA TRỰC TIẾP FILE NÀY - Chạy: node scripts/sync-inline-workers.cjs
...
// ============= 2048 AI WORKER =============
const AI_2048_CODE = `// 2048 AI Worker using Alpha-Beta Search
...
// ============= CARO AI V1 (MTDF) =============
const CARO_V1_CODE = `// Caro AI Worker - Minimax with Alpha-Beta Pruning
let GameBoard;
let fc = 0;
let Rows;
let Columns;
...
function TIMEOUT() {
  return Date.now() - startTime >= MaximumTimeForMove;
}

function iterative_mtdf(Board) {
  const restrictions = Get_restrictions(Board);
  let guess = evaluate_state(Board, 1, hash(GameBoard), [
    0,
    0,
    Rows - 1,
    Columns - 1,
...
function negamax(
  ...
  let availSpots =
    depth === MaximumDepth
      ? Get_last_best()
      : BoardGenerator(restrictions, newBoard, player);
...
onmessage = function (e) {
  ...
  MaximumTimeForMove = e.data[2] || 2000;
  if (Board) {
    GameBoard = Board;
...
// ============= CARO AI V2 (Fixed Depth Negamax) =============
const CARO_V2_CODE = `// Caro AI Worker V2 - Iterative Negamax with fixed depth
let GameBoard;
let fc = 0;
let Rows;
let Columns;
...
// ============= CARO AI V3 (Timeout Negamax) =============
const CARO_V3_CODE = `// Caro AI Worker V3 - Iterative Negamax with Timeout
let GameBoard;
let fc = 0;
let Rows;
let Columns;
...
function eval_board(Board, pieceType, restrictions) {
  let score = 0;
  const min_r = restrictions[0],
    min_c = restrictions[1],
    max_r = restrictions[2],
    max_c = restrictions[3];
  for (let row = min_r; row < max_r + 1; row++) {
    for (let column = min_c; column < max_c + 1; column++) {
      if (Board[row][column] == pieceType) {
        let block = 0,
...
const Cache = new Map();
const StateCache = new Map();
let MaximumDepth, MaximumTimeForMove, startTime;
let CacheHits = 0,
  CacheCutoffs = 0,
  CachePuts = 0,
  StateCacheHits = 0,
...
function search(player, time) {
  ...
  const t0 = performance.now();
  const bestmove = iterative_negamax(player, GameBoard);
  const t1 = performance.now();
  Cache.clear();
  StateCache.clear();
  return { bestmove, time: (t1 - t0) / 1000 };
...
onmessage = function (e) {
  ...
  if (Board) {
    ...
    fc = 0;
    Table_init();
    postMessage(search(-1, time));
  }
};
`;

// ============= CARO AI V4 (NegaScout Fixed Depth) =============
const CARO_V4_CODE = `// Caro AI Worker V4 - NegaScout (Principal Variation Search)
let GameBoard;
let fc = 0;
let Rows;
let Columns;
...
function negascout(
  ...
  if (CacheNode !== undefined && CacheNode.depth >= depth) {
    ...
    if (CacheNode.Flag === -1) alpha = Math.max(alpha, score);
    else if (CacheNode.Flag === 1) beta = Math.min(beta, score);
    if (alpha >= beta) {
      CacheCutoffs++;
      return score;
    }
...
function search(player, depth) {
  const t0 = performance.now();
  const bestmove = iterative_negascout(player, GameBoard, depth);
  const t1 = performance.now();
  Cache.clear();
  StateCache.clear();
  return { bestmove, time: (t1 - t0) / 1000 };
...
onmessage = function (e) {
  ...
  if (Board) {
    ...
    let sum = 0;
    for (let x = 0; x < Rows; x++)
      for (let y = 0; y < Columns; y++) if (GameBoard[x][y] !== 0) sum++;
    if (sum === 0) {
      const center = Math.floor(Rows / 2);
      postMessage({ bestmove: { i: center, j: center, score: 0 }, time: 0 });
      return;
...
// ============= CARO AI V5 (NegaScout + Timeout) =============
const CARO_V5_CODE = `// Caro AI Worker V5 - NegaScout + Iterative Deepening + Timeout (Ultimate)
let GameBoard;
let fc = 0;
let Rows, Columns;
const WIN_DETECTED = false;
...
function search(player, time) {
  ...
  MaximumTimeForMove = time;
  const t0 = performance.now();
  const bestmove = iterative_negascout(player, GameBoard);
  const t1 = performance.now();
  Cache.clear();
  StateCache.clear();
  return { bestmove, time: (t1 - t0) / 1000 };
...
// ============= CARO AI ULTIMATE =============
const CARO_ULTIMATE_CODE = `// Caro AI Ultimate - Siêu phàm + Nhanh
